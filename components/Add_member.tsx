...
const AddMember = ({ route, navigation }: any) => {
        ...
        const find_user = async (username: string) => {
            ...
            if (searchTerm.length > 0) {
                const searchUsers = debounce(async (term) => {
                    try {
                        const response = await fetch(`${API_HOST}/search_member/?q=${term}&page=1&gym=${route.params.gymname}`, {
                            method: 'get',
                            headers: {
    ...
    useEffect(() => {
        ...
        if (scrollEffect && searchTerm.length > 0) {
            const searchUsers = debounce(async (term) => {
                try {
                    const response = await fetch(`${API_HOST}/search_member/?q=${term}&page=${page + 1}&gym=${route.params.gymname}`, {
                        method: 'get',
                        headers: {
                            'Content-Type': 'application/json',
                            'Authorization': `Token ${await AsyncStorage.getItem('@token')}`,
    ...
    const savetrainer = async (username: string) => {
        const addtrainer = async (username: string) => {
            console.log(route.params.gymname);
            const response = await fetch(`${API_HOST}/add_member_request/?gym=${route.params.gymname}&username=${username}`, {
                method: 'get',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Token ${await AsyncStorage.getItem('@token')}`,
        ...
        Alert.alert('Add Trainer', 'Are you sure you want to add to member ?', [
            {
                text: 'Cancel',
            ...
            <FlatList style={{ backgroundColor: bg_color }}
                ...
                renderItem={({ item }) =>
                    <TouchableOpacity activeOpacity={0.6} style={styles.search_result}>
                        ...
                        {item.already_added ? <View style={styles.plus_icon_holder}><Text style={styles.plus_icon}>Trainer</Text></View> :
                            <>{item.already_member ? <View style={styles.plus_icon_holder}><Text style={styles.plus_icon}>Member</Text></View> :
                                <>{item.is_requested ? <TouchableOpacity style={styles.plus_icon_holder} onPress={() => removemember(item.username)}><Text style={styles.plus_icon}>Requested</Text></TouchableOpacity> :
                                    <TouchableOpacity style={styles.plus_icon_holder} onPress={() => savetrainer(item.username)}><Text style={styles.plus_icon}>+</Text></TouchableOpacity>}</>}</>}
                    </TouchableOpacity>}
            />
